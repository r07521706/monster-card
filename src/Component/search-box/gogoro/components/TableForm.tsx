...
class TableForm extends PureComponent<TableFormProps, TableFormState> {
  ...
  newMember = () => {
    const { data = [] } = this.state;
    const newData = data.map(item => ({ ...item })); //複製
    console.log(newData);
    console.log(data);
    newData.push({
  ...
  handleKeyPress(e: React.KeyboardEvent, key: string) {
    if (e.key === 'Enter') {
      this.saveRow(e, key);
        const { data } = this.state;
        const { onChange } = this.props;
      console.log('key_press',data);
      console.log('key_press',onChange);
    }
  }
  ...
  saveRow(e: React.MouseEvent | React.KeyboardEvent, key: string) {
    ...
    setTimeout(() => {
      ...
      dispatch({type:'formAdvancedForm/addName',payload:data});
      console.log('okkkk');
      // console.log(data);
      const { onChange } = this.props;
      if (onChange) {
  ...
  cancel(e: React.MouseEvent, key: string) {
    ...
    const { data = [] } = this.state;
    const newData = [...data];
    const originData = newData.map(item => {
      if (item.key === key) {
